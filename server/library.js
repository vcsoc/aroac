...
export function installLibrary(app, db) {
  ...
  app.get("/api/address-book", (_req, res) =>
    res.json(
      db.prepare("SELECT * FROM address_contacts ORDER BY name,callsign").all(),
    ),
  );
  app.post("/api/address-book", (req, res) => {
    try {
      const p = contact(req.body),
        r = db
          .prepare(
            "INSERT INTO address_contacts(name,callsign,email,grid,notes) VALUES(?,?,?,?,?)",
          )
          .run(p.name, p.callsign, p.email, p.grid, p.notes);
  ...
  app.put("/api/address-book/:id", (req, res) => {
    try {
      ...
      if (
        !db
          .prepare(
            "UPDATE address_contacts SET name=?,callsign=?,email=?,grid=?,notes=? WHERE id=?",
          )
          .run(p.name, p.callsign, p.email, p.grid, p.notes, req.params.id)
          .changes
      )
  ...
  app.delete("/api/address-book/:id", (req, res) => {
    db.prepare("DELETE FROM address_contacts WHERE id=?").run(req.params.id);
    res.json({ ok: true });
  });
  app.get("/api/saved-search", (req, res) => {
    ...
    const contacts = db
      .prepare("SELECT * FROM address_contacts")
      .all()
      .filter(matches)
      .slice(0, 20);
  ...
  app.get("/api/library/export", (req, res) => {
    const contacts = db
      .prepare("SELECT name,callsign,email,grid,notes FROM address_contacts")
      .all();
    if (req.user)
      contacts.push(
        ...db
    ...
    res.json({
      ...
      pins: db
        .prepare("SELECT label,name,callsign,lat,lng,notes FROM pins")
        .all(),
      contacts,
      qsos: req.user
        ? db
      ...
      for (const [key, normalize, table] of [
        ...
        for (const row of data[key]) {
          ...
          if (key === "pins")
            ...
            db.prepare(
              "INSERT INTO address_contacts(name,callsign,email,grid,notes) VALUES(?,?,?,?,?)",
            ).run(row.name, row.callsign, row.email, row.grid, row.notes);
          else
            db.prepare(
