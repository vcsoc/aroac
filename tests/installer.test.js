...
const linuxOnly = {
  skip:
    ...
    "Linux installer requires Linux filesystem semantics",
};
...
base='https://github.com/vcsoc/aroac/releases/download/v1.2.3/'
name='AROAC-1.2.3.AppImage'
fixture_sums='AROAC-1.2.3-SHA256SUMS.txt'
...
test(
  "Linux installer installs atomically into user paths, preserves a previous executable and leaves app data alone",
  linuxOnly,
  () => {
...
for (const scenario of ["checksum", "url", "arm"])
  test(
    "Linux installer rejects " + scenario + " before installation",
    linuxOnly,
    () => {
