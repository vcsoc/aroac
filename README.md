...
- **Locations/contacts transfer:** Locations, Contacts, Export and Import are icon buttons on a shared toolbar, followed by a question-mark transfer tooltip. Export/import versioned OAR JSON through native file dialogs in Settings → Data or Saved locations. Exports include all saved pins/address-book contacts and, when signed in, your QSO logbook and legacy saved local operator contacts. Import previews counts, merges records atomically and skips exact duplicates; importing QSOs is explicitly optional and requires sign-in. It never creates accounts or imports passwords/sessions. Imports accept up to 10,000 records per category and an 8 MB file. Full SQLite backup remains available for larger libraries.
...
- **Back up database** creates a consistent SQLite snapshot through a native save dialog. The database and backups are not encrypted; protect your device and backup files.
