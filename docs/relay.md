...
OS-protected key storage is mandatory. Linux `basic_text` is rejected; AROAC's optional plaintext *login session* fallback is never reused for relay keys. If the OS vault cannot be used/unlocked, transport fails closed and the local workspace still works. Configuration, identities, peer pins, outbox and retained messages are stored encrypted in owner-only files under `private-relay/` in application data. This does not protect a compromised OS account or application process.
...
- This is a new integration, not an independently audited messaging protocol. The primitive implementation is libsodium; do not confuse that with an audit of AROAC's integration.
...
Before Electron initializes, AROAC selects Secret Service/libsecret on non-KDE Linux desktops, including Hyprland and Sway. KDE/Plasma keeps Chromium's version-aware KWallet selection. Explicit `--password-store` options are preserved; `basic_text` is always rejected for relay data. Windows retains native account protection and macOS retains Keychain. AROAC does not install, unlock or replace OS vaults silently, switch an existing user's backend after decryption failure, or store relay secrets in plaintext.

Settings → Relay shows the selected backend and actionable OS-specific guidance. After unlocking the original vault, use **Retry credential storage** to reload saved configuration/identity without generating new keys or granting consent. If initialization remains unavailable, use **Restart AROAC** (save unfinished edits first); launch arguments are retained. Failed reads are not retried by polling. Unreadable settings cannot be overwritten by import. Successfully decrypted settings rejected only by URL policy may be replaced with valid HTTPS settings. Encryption failures leave the previous sealed file intact.

Absent Linux provider: install/enable a Secret Service provider such as GNOME Keyring, or KDE Wallet for KDE. Locked/denied vault: unlock and authorize AROAC, then retry. AROAC cannot reliably distinguish a locked vault, denied access, a lost key, or corrupt ciphertext; diagnostics must not promise recovery. Keep existing sealed data; never delete it as an unlock workaround. Lost OS keys may make data unrecoverable. Explicit unsupported/basic storage overrides must be removed from the launcher before a full restart.
